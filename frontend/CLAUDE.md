...
- Styling: **SCSS**, plus global **Bootstrap 5** and **Font Awesome** (wired via `angular.json` styles/scripts, not imports). `flowbite` is also a dependency. Bootstrap Icons (`bi bi-*`) are used throughout templates. Shared design tokens live in each tree's `shared/scss/_variables.scss` (teal-based palette; `.btn-teal-action`, `.text-teal`, `fs-7` are recurring custom classes), pulled in via `@use '...' as *` (not the deprecated `@import`) so bare `$variable` references keep working unqualified.
